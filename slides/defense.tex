...
\author[F. P\'erez Mill\'an]{Fernando P\'erez Mill\'an}
...
\begin{frame}{Data and Forecasting Setup}
...
\end{itemize}
\end{block}
\begin{block}{Forecast exercise (Test set)}
\begin{itemize}
\item Out-of-sample window: Jan 2025--Apr 2025 (81 observations).
\item 1-day-ahead density, VaR (2.5\%), and ES forecasts.
...
\begin{frame}{Modeling Objective}
\begin{itemize}
\item Returns are modeled empirically, not only theoretically.
...
\end{columns}
\vspace{0.5em}
\begin{block}{Baseline return model}
\[
...
\begin{frame}{Volatility Specifications}
\begin{itemize}
...
\item Density diagnostics (PIT histograms, KS on \(\Phi^{-1}(u_{t+1})\), Berkowitz, DM).
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{frame}{EVT-Based Tail Forecasting}
\small
\[
\text{VaR}_{z,t+1}^{p}=u-\frac{\beta}{\xi}\left\{1-\left[\frac{T}{N_u}(1-q)\right]^{-\xi}\right\},\quad
...
\large \githubicon\ \href{https://github.com/ferpmillan/tesis-tex}{github.com/ferpmillan/tesis-tex}
\end{frame}
